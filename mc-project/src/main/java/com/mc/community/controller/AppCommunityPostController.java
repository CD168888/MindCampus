...
public class AppCommunityPostController extends BaseController {
    ...
    public TableDataInfo list(CommunityPost communityPost) {
        ...
        // 处理匿名显示逻辑（APP端需要隐藏匿名用户的真实信息）并设置点赞状态
        for (CommunityPost post : list) {
            communityPostService.processAnonymousDisplay(post);
            // 检查当前用户是否点赞过该帖子
            if (currentUserId != null) {
                CommunityPostLike like = communityPostLikeMapper.selectLikeByPostAndUser(post.getPostId(), currentUserId);
    ...
    public R<CommunityPost> getDetail(@PathVariable Long postId) {
        ...
        Long currentUserId = SecurityUtils.getUserId();
        if (currentUserId != null) {
            CommunityPostLike like = communityPostLikeMapper.selectLikeByPostAndUser(postId, currentUserId);
            post.setIsLiked(like != null);
        }
